...
##
## _use_pitch: If false, rotation will only happen on the Y axis. If true, rotation will happen on all axis.
## _lerp_val: The speed rotation should happen at. 0.0, no rotation. 0.1, slow. 1.0, instant.
func face_point(node: Node3D, point: Vector3, _use_pitch: bool = false, _lerp_val: float = 0.0) -> void:
